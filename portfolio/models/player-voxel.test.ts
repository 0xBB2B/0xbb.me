...
for (const [name, create, height] of [
  ['base geometry', createPlayerVoxel, 3], ['black outfit', createBlackOutfitPlayerVoxel, 2.4],
] as const) {
  test(`${name} is complete, grounded and uses geometry colors rather than image textures`, () => {
    const model = create();
    try {
      const bounds = new THREE.Box3().setFromObject(model);
      expect(bounds.max.y - bounds.min.y).toBeCloseTo(height, 6);
