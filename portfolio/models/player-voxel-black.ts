...
// Visible details use geometry and vertex colors.
export function createBlackOutfitPlayerVoxel() {
  const model = new THREE.Group();
  model.name = 'FUBUKI_Minecraft_Black_Outfit';
  model.userData = { variant: 'Black outfit', artwork: 'Geometry and vertex colors; no image textures' };
  ...
  function block(parent: THREE.Object3D, name: string, hex: number,
    x: number, y: number, z: number, w: number, h: number, d: number, shaded = true) {
    const mesh = new THREE.Mesh(paint(new THREE.BoxGeometry(w, h, d), hex, shaded), material);
    mesh.name = name;
  ...
  torso.name = 'Torso';
  model.add(torso);
  block(torso, 'Cropped_black_jacket', palette.jacket, 0, 20, 0, 8, 8, 4);
  ...
  for (const side of [-1, 1]) {
    ...
    block(arm, 'Black_cuff', 0x1b222c, 0, -9.05, 0, 3.25, 1.2, 4.05, false);
  ...
  block(torso, 'Bare_waist', 0xe8b9a8, 0, 14.45, 0, 7.8, 3.1, 3.95, false);
  block(torso, 'Waist_shade', 0xc88f88, 2.9, 14.45, 2, 1.3, 3, 0.08, false);
  block(torso, 'Black_crop_top', 0x151a23, 0, 19.1, 2.065, 4.9, 6.1, 0.1, false);
  block(torso, 'Top_fold', 0x41434b, 0, 20.3, 2.13, 4.5, 0.6, 0.05, false);
  block(torso, 'Top_hem', 0x090f18, 0, 16.25, 2.14, 4.95, 0.4, 0.06, false);
  block(torso, 'Exposed_neckline', 0xe8b9a8, 0, 23.4, 2.08, 3.4, 1.2, 0.12, false);
  block(head, 'Left_blue_earring', 0x4cc9df, -4.55, 25.8, 3.2, 0.65, 1, 0.55, false);
  for (const side of [-1, 1]) {
    const leg = model.getObjectByName(side < 0 ? 'Left_leg' : 'Right_leg')!;
    block(leg, 'Platform_sole', 0x0b1420, 0, 0.32, 0.5, 4.3, 0.64, 5.25, false);
    for (const y of [1.6, 4.5]) {
      block(leg, 'Boot_strap', 0x46515e, 0, y, 2.16, 4.1, 0.38, 0.16, false);
      block(leg, 'Strap_buckle', 0xaab9c2, side * 1.3, y, 2.27, 0.48, 0.5, 0.1, false);
    }
    for (const y of [2.2, 2.95, 3.7]) {
      for (const direction of [-1, 1]) {
        const lace = block(leg, 'Crossed_boot_lace', 0x7a8794, 0, y, 2.24, 1.65, 0.16, 0.12, false);
