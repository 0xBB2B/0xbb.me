...
访客可通过语言入口在中文和英文之间切换。
切换覆盖界面提示、NPC 对话、资料速览及故障说明。
切换语言不重置主人公位置或当前对话段落，不保存跨刷新状态。

## 约束
- C-1：当访客首次进入或刷新主页时，系统应默认使用英文并从探索起点开始，不恢复上次刷新前的语言或位置。
- C-2：当访客切换中文或英文时，系统应同步切换可见界面提示、场景名称、NPC 介绍、资料速览和故障说明；技术、项目、品牌名称和链接目标保持其原有身份与含义。
- C-3：如果访客在探索中、对话中或资料速览中切换语言，系统应保持主人公位置；已打开的阅读面板不关闭，NPC 对话仍处于语义对应的同一段落。
- C-4：如果图形不可用，系统仍应允许切换中文与英文，故障说明和完整资料采用所选语言，链接仍可使用。

## 例子
访客以英文进入工坊，在导师介绍 Go 的段落切换中文；主人公仍留在导师旁，对话继续显示 Go 主题的中文段落，而不是回到城镇或重开第一段。
