...
---
## 1. 目标
在M3获授权后，验证完整静态交付、双语触屏、无位图与桌面性能，更新准确说明；不自动发布。
## 2. 业务规则
...
- README逐项必须出现：FUBUKI_BB 的 HD-2D 横向探索个人主页；A/D 或左右方向键行走，E 主动交谈，触屏左右/交谈；中英切换与资料速览；图形失败仍可读；bun run dev、bun test、bun run build、bun run preview；图形代码化且参考图不发布；实际开源来源与许可。不再出现Tactical HUD首页、独立/game/游戏、节奏光剑操作或音乐功能说明。
- 生产构建真实文件和浏览器资源内容须无位图、编码位图或SVG包裹位图，原图/截图不入dist；使用内存负例测检测有辨别力，不把图片夹具放public。
