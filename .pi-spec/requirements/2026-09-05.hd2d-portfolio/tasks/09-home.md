...
## 8. 验证方式
- 测试写入授权：仅新增或修改 `/Users/bb/Projects/0xbb.me/App.test.tsx`；不得修改生产文件或任务字段。
- 独立验证命令：`cd /Users/bb/Projects/0xbb.me && bun test ./App.test.tsx`。
- 公开验证入口：`App.tsx` 的 App 组件输出与网页 `/`；输入刷新、语言切换、阅读状态及三种指定视口，按业务规则观察英文起点、位置连续性和可用入口，不以源代码字符串代替页面行为。
