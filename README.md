...
- 空格跳跃适用于黑装和A服装，不提供触屏跳跃按钮；触屏模式不触发跳跃。按住空格不会自动连跳；移动起跳沿用当前迈步时的手脚前后顺序，平滑过渡到前后迈开双腿、反向摆臂的姿势；原地起跳使用反向的默认顺序。空中保持姿势，接近落地时收拢，不在空中循环走路/跑步动作。跳跃不能越过关闭的门或道路端点；阅读、开门、换装和彩蛋期间不能起跳。
...
portfolio/models/             正式黑装主角的基础几何、造型与模型测试
...
- 临时设计对比目录已删除。角色源码和必要测试归入`portfolio/models/`及`tests/`，不保留无用的参考图、预览页面或GLB导出。
